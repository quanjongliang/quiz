...
import { Reply } from 'types/template/user-profile';
...
export function getUsersListStyle2() {
    return async () => {
        try {
            ...
            dispatch(slice.actions.getUsersListStyle2Success(response?.data?.data));
        } catch (error) {
            dispatch(slice.actions.hasError(error));
        }
    };
...
// export function getUserKechoDetailsById(id:number){
//     return async () => {
//         try
//     }
// }
