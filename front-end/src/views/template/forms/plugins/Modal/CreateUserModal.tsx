...
import AuthRegisterKecho from 'views/template/pages/authentication/auth-forms/AuthRegisterKecho';
// generate random
function rand() {
...
const Body = React.forwardRef(({ modalStyle: _modalStyle, handleClose, pagination }: BodyProps, ref: React.Ref<HTMLDivElement>) => (
    <div ref={ref} tabIndex={-1}>
        ...
        >
            <CardContent>
                <AuthRegisterKecho handleclose={handleClose} pagination={pagination} />
...
export default function CreateUserModal({ pagination }: CreateUserModalProps) {
    ...
    return (
        <Grid container justifyContent="flex-end">
            ...
            <Tooltip title="Create New User" placement="top">
                <IconButton color="primary" size="large" onClick={handleOpen}>
                    <AddIcon sx={{ fontSize: '1.3rem' }} />
                </IconButton>
            </Tooltip>
            <Modal open={open} onClose={handleClose} aria-labelledby="simple-modal-title" aria-describedby="simple-modal-description">
                <Body modalStyle={modalStyle} handleClose={handleClose} pagination={pagination} />
            </Modal>
        </Grid>
    );
}
